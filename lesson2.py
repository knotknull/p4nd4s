...
def l2_loc() -> None:
    ...
    print(
        "iloc[:4,1]", reviews.iloc[:4, 1]
    )  # This gets first five rows and the first column
    print("iloc[[98,99,100],1]", reviews.iloc[[98, 99, 100], 1])  # pass a lsit
    print(
        "iloc[-5:]", reviews.iloc[-5:]
    )  # hey look negative numbers!! starts from end and works backward

    ## loc index location
    print(
        "loc[[0,1,10,100],['country','province','region_1','region_2']]",
    ...
    )  # query countries and price

    print(
        "reviews.loc[reviews.price.notnull()]", reviews.loc[reviews.price.notnull()]
    )  # not null prices

    print(
        "reviews.loc[reviews.price.isnull()]", reviews.loc[reviews.price.isnull()]
    )  # null prices
    return None
